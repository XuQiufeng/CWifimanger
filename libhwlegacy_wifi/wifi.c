...
#include "libwpa_client_static/wpa_ctrl.h"
...
int wifi_load_driver()
{
    ...
    sched_yield();
    while (count-- > 0) {
        ALOGD("%s:wifi driver loading...", __func__);
        if (property_get(DRIVER_PROP_NAME, driver_status, NULL)) {
...
int wifi_start_supplicant(int p2p_supported)
{
    ...
    ALOGD("%s:Starting WPA_Supplicant...", __func__);
    while (count-- > 0) {
#ifdef HAVE_LIBC_SYSTEM_PROPERTIES
        if (pi == NULL) {
            pi = __system_property_find(supplicant_prop_name);
...
int wifi_stop_supplicant(int p2p_supported)
{
    ...
    ALOGD("%s:Stopping WPA_Supplicant...", __func__);
    while (count-- > 0) {
        if (property_get(supplicant_prop_name, supp_status, NULL)) {
...
int wifi_connect_to_supplicant(const char *ifname)
{
    ...
    ALOGD("%s:Connecting to supplicant %s...", __func__, ifname);
...
int wifi_ctrl_recv(int index, char *reply, size_t *reply_len)
{
    ...
    if (rfds[0].revents & POLLIN) {
         ...
         */
        if (index == SECONDARY) {
            ALOGD("%s: close sockets %d", __func__, index);
...
{
    ALOGD("%s: Sending command [%s] to [%s]", __func__, command, ifname);
    if (is_primary_interface(ifname)) {
